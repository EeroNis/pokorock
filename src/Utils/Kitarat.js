import kuva1 from '../assets/guitar-g6227cd99c_1920.jpg';
import kuva2 from '../assets/guitar-gd4bfbb1c4_1920.jpg';
import kuva3 from '../assets/guitar-gef29ff2eb_1920.jpg';
import kuva4 from '../assets/guitar-g66bcb142d_1920.jpg';
import kuva5 from '../assets/painted-guitar-g381ae4243_1920.jpg';
import kuva6 from '../assets/guitar-gb3427d1e0_1920.jpg';
import kuva7 from '../assets/guitar-gd06a70b1b_1920.jpg';

export const Kitaratuotteet = [
  {
    kuva: kuva1,
    otsikko: 'Gibson Les Paul',
    hinta: 'Hinta: 1200€',
    id: 1,
    teksti:
      'Gibson Les Paul on unelmien valinta. Tästä soitin johon et tule pettymään. Runko on tehty mahongista joka kestää vuosien renkuttamisen. Kaula on tehty ruusupuusta joten ei huolta et saisi sitä rikki millään. Kitarasta löytyy 24 nauhan väliä joten hyvin saa swengattua taajuutta kattoon. Mikit ovat Gibson 490R mallistoa jsekä löytyy myös 498T humbuukkerit. Valmistettu meidän kaupassa.',
  },
  {
    kuva: kuva2,
    otsikko: 'Harley Benton ST',
    hinta: 'Hinta: 80€',
    id: 2,
    teksti:
      'Harley Benton ST ei petä. Tästä luutusta löytyy sitä jotain. Jos ostat nyt saat toisen kaupan päälle. 22 nauhaväliä joten sopivan korkealle pääsee renkuttamaan. Värivaihtoehtoja löytyy joten kysy pois mikä voisi silmää miellyttää. 2 vollyymi nappullaa löytyy joten maistuu kilauttaa vaikka kaverillekki. Kitara on valmistettu vaahterasta joka soinnissa on erinomainen materiaali.',
  },
  {
    kuva: kuva3,
    otsikko: 'Squier Starcaster',
    hinta: 'Hinta: 500€',
    id: 3,
    teksti:
      'Squier Starcaster Active maistuu vaikka Marekpokollekkin. Kaverisi tulevat tästä katelliseksi joten osta hyvä tuote pois. Runko on laminoitua vaahteraa vai oliko se sittenkin paahdettua sellaista. Löytyy onneksi myös jumbomutteri kultaupotuksilla joten blingblingiä löytyy tarpeeksi vaativammallekkin soittajalle. Kielet ovat nikkelipinnoitteella varustettuja.',
  },
  {
    kuva: kuva4,
    otsikko: 'Gibson Les Paul',
    hinta: 'Hinta: 2400€',
    id: 4,
    teksti:
      'Gibson Les Paul on unelmien valinta. Tästä soitin johon et tule pettymään. Runko on tehty mahongista joka kestää vuosien renkuttamisen. Kaula on tehty ruusupuusta joten ei huolta et saisi sitä rikki millään. Kitarasta löytyy 24 nauhan väliä joten hyvin saa swengattua taajuutta kattoon. Mikit ovat Gibson 490R mallistoa jsekä löytyy myös 498T humbuukkerit. Valmistettu meidän kaupassa.',
  },
  {
    kuva: kuva5,
    otsikko: 'Harley Benton ST',
    hinta: 'Hinta: 300€',
    id: 5,
    teksti:
      'Harley Benton ST ei petä. Tästä luutusta löytyy sitä jotain. Jos ostat nyt saat toisen kaupan päälle. 22 nauhaväliä joten sopivan korkealle pääsee renkuttamaan. Värivaihtoehtoja löytyy joten kysy pois mikä voisi silmää miellyttää. 2 vollyymi nappullaa löytyy joten maistuu kilauttaa vaikka kaverillekki. Kitara on valmistettu vaahterasta joka soinnissa on erinomainen materiaali.',
  },
  {
    kuva: kuva6,
    otsikko: 'Squier Starcaster',
    hinta: 'Hinta: 4500€',
    id: 6,
    teksti:
      'Squier Starcaster Active maistuu vaikka Marekpokollekkin. Kaverisi tulevat tästä katelliseksi joten osta hyvä tuote pois. Runko on laminoitua vaahteraa vai oliko se sittenkin paahdettua sellaista. Löytyy onneksi myös jumbomutteri kultaupotuksilla joten blingblingiä löytyy tarpeeksi vaativammallekkin soittajalle. Kielet ovat nikkelipinnoitteella varustettuja.',
  },
  {
    kuva: kuva7,
    otsikko: 'Unskin vasurin  kitara',
    hinta: 'Hinta: 400€',
    id: 7,
    teksti:
      'Unskin singature luuttu klassiseen soittoon. 22 nauhan väliä riittää jannulle kuin jannulle. Yläosa massiivista kuusta ja pohja vaahteraa. Käytetty myös pähkinäpuuta. Luonnollisen saatini luonnonhartsilla. Kaula leppää. Aloittelevaan käyttöön kuin kokeneelle konkarille mainio instrumentti.',
  },
];
